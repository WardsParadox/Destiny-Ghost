...
parser.add_argument("--config",
                    ...
                    default="/Libary/Application Support/com.github.wardsparadox.Destiny-Ghost/config.json")
...
def main():
    ...
    if not any(vars(args).values()):
        ...
        if data is None:
            logging.debug("Lookup Command returned None, Trying to prepend S to serial")
            data = get_device_data("S"+serialnumber(),
                                   server_settings["server"],
                                   server_settings["user"],
                                   server_settings["password"],
                                   server_settings["database"])
            logging.info("Got device data from server (Prepended S to serial)!\n%s", data)
